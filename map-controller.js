...
$scope.add_marker = function($http) {
 ...
 geocoder.geocode({ 'address': address }, function (results, status) {
  ...
  addmarker = [];
  for(var j=0; j < address; j++) {
    var marker_values = addmarker.push(address[j].latitude, address[j].longitude, address[j].value);
    console.log(longitude);
    $http.get("goa-bikes.json").push(marker_values);
  }  
